...
loss_type = 'ragan'  # rsgan lsgan ragan vanilla wgan hinge, for Discriminator (CatGAN)
...
log_filename = strftime("log/log_{}_{}".format(log_time_str, '+'.join(
    [m[:3] for m in
     mu_temp.split()])))
temp_log_filename = strftime("temp_log/temp_log_{}_{}".format(log_time_str, '+'.join(
    [m[:3] for m in
     mu_temp.split()])))
...
if os.path.exists(temp_log_filename + '.txt'):
    i = 2
    while True:
        if not os.path.exists(temp_log_filename + '_%d' % i + '.txt'):
            temp_log_filename = temp_log_filename + '_%d' % i
            break
        i += 1
temp_log_filename = temp_log_filename + '.txt'
...
pretrained_gen_path = pretrain_root + 'gen_MLE_pretrain_{}_{}_sl{}_sn{}_epoch{}_time{}_batch{}_eval{}.pt'.format(
    run_model, model_type, max_seq_len,
    samples_num, MLE_train_epoch, time.strftime("%Y%m%d%H%M"), batch_size, eval_b_num)
pretrained_dis_path = pretrain_root + 'dis_pretrain_{}_{}_sl{}_sn{}_time{}_batch{}_eval{}.pt'.format(run_model,
                                                                                                     model_type,
                                                                                                     max_seq_len,
                                                                                                     samples_num,
                                                                                                     time.strftime(
...
def init_param(opt):
    global run_model, model_type, loss_type, CUDA, device, data_shuffle, samples_num, vocab_size, \
        ...
        use_nll_oracle, use_nll_gen, use_nll_div, use_bleu, use_self_bleu, use_clas_acc, use_ppl, temp_log_filename,mu_temp,fn_mu_temp
    ...
    tips = opt.tips
    # temp_log_filename = opt.temp_log_filename
