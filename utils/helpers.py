...
def create_temp_logger(name, silent=False, to_disk=False, log_file=None):
    log = logging.getLogger(name)
    log.setLevel(logging.DEBUG)
    log.propagate = False
    formatter = logging.Formatter(fmt='%(message)s', datefmt='%Y/%m/%d %I:%M:%S')
    if not silent:
        ...
        log.addHandler(ch)
    if to_disk:
        log_file = log_file if log_file is not None else strftime("temp_log/temp_log_%m%d_%H%M.txt", gmtime())
        if type(log_file) == list:
            for filename in log_file:
                fh = logging.FileHandler(filename, mode='w')
                fh.setLevel(logging.INFO)
...
def get_fixed_temperature(temper, i, N, adapt):
    ...
    # print('******* chosen evolution choice as : '+str(adapt))
