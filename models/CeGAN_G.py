...
class CeGAN_G(LSTMGenerator):
    def __init__(self, mem_slots, num_heads, head_size, embedding_dim, hidden_dim, vocab_size, max_seq_len, padding_idx,
                 gpu=False):
        super(CeGAN_G, self).__init__(embedding_dim, hidden_dim, vocab_size, max_seq_len, padding_idx, gpu)
        self.name = 'cegan'

        # self.temperature = 1.0  # init value is 1.0
        # this temperature will be a evolutionarily value in future.
    ...
    def step(self, inp, hidden):
        """
        RelGAN step forward this file also as similar as EVO-GAN
        Is there really useful??
    ...
    def sample(self, num_samples, batch_size, one_hot=False, start_letter=cfg.start_letter):
        """
        Sample from CeGAN Generator
        - one_hot: if return pred of CeGAN, used for adversarial training
        :return:
    ...
    def add_gumbel(o_t, eps=1e-10, gpu=cfg.CUDA):
        ...
        u.uniform_(0, 1)
        # Why this function was been used?
        # ke evo-gan
